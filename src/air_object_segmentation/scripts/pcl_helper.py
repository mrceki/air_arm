...
def get_min_max_z(XYZRGB_cloud):
...
def compare_color(h, s, v):

    if((0.0<=h and h<=0.05) or (0.94<=h and h<=1.0)):
#        return True
        if( 0.27<s<1.0 and 0.27<v<1.0):
            return True
    return False

def calculate_mean(XYZ_cloud):
    sum_x = 0.0
    sum_y = 0.0
    sum_z = 0.0
    size = XYZ_cloud.size
...
#        print(hsv[0], hsv[1], hsv[2])
        if(compare_color(hsv[0], hsv[1], hsv[2])):
            points_list.append([data[0], data[1], data[2], data[3]])
    filtered_cloud.from_list(points_list)
