...
geometry_msgs::PosePtr target_pose = nullptr;
...
void convertTFtoPose(tf::StampedTransform &qr_transform,
  ...
  pose.position.x = qr_transform.getOrigin().x() + 0.01 - 0.15;
  pose.position.y = qr_transform.getOrigin().y() - 0.03;
  pose.position.z = qr_transform.getOrigin().z() + 0.1 + 0.01;
  //base_transform.
...
bool AirDemo::setGoal(geometry_msgs::Pose &goal_pose, std::string goal_tf_name, std::string text){
  ...
  adjustGoalPose(goal_pose, goal_tf);
...
AirDemo::AirDemo(ros::NodeHandle nh):move_group_(PLANNING_GROUP_),
                   visual_tools_(VISUALIZATION_FRAME_),
                   qb_hand_client_(QBHAND_ACTION, true), nh_(nh){
      ...
      if(setPlan(plan, pick_pose)){
        ...
        if(setPlan(plan, second_pose)){
          ...
          if(setGoal(place_pose, "/fiducial_100", "place_pose")){
            ...
            place_pose.position.x += 0.05;
            place_pose.position.y -= 0.025;
            place_pose.position.z += 0.11;

            tf::Quaternion q_orig, q_rot, q_new;

            q_orig.setX(place_pose.orientation.x);
            q_orig.setY(place_pose.orientation.y);
            q_orig.setZ(place_pose.orientation.z);
            q_orig.setW(place_pose.orientation.w);

            q_rot.setRPY(0, 0, 0.2);

            q_new = q_rot*q_orig;
            q_new.normalize();

            place_pose.orientation.x = q_new.getX();
            place_pose.orientation.y = q_new.getY();
            place_pose.orientation.z = q_new.getZ();
            place_pose.orientation.w = q_new.getW();
...
void AirDemo::pickDemo(){
  ...
  std::string goal_tf_name("/ball_tf");
  bool run_demo = true;
  bool result;
  char user_input;
...
void goalCB(const geometry_msgs::PoseStamped::ConstPtr& msg){
    target_pose.reset(new geometry_msgs::Pose());
